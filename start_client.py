...
def run_single_inference(server_address: str, model_name: str, split_point: int, 
                        batch_size: int = 1) -> Tuple[torch.Tensor, Dict]:
    """Run a single inference with specified split point"""
...
def run_batch_processing(server_address: str, model_name: str, split_point: int, 
                        batch_size: int, num_batches: int) -> List[Dict]:
...
def main():
    ...
    parser.add_argument('--split-point', type=int, default=2,
                       help='Split point for model partitioning (default: 2)')
    parser.add_argument('--batch-size', type=int, default=1,
                       help='Batch size for inference (default: 1)')
    parser.add_argument('--num-batches', type=int, default=1,
                       help='Number of batches to process (default: 1)')
    parser.add_argument('--test-splits', type=str,
                       help='Comma-separated split points to test (e.g., "0,1,2,3,4")')
    ...
    try:
        ...
        # Run batch processing
            ...
            timings_list = run_batch_processing(
                args.server_address, args.model, args.split_point, 
        ...
        # Run single inference
            ...
            logger.info(f"🎯 Running single inference with split point {args.split_point}")
            
            result, timings = run_single_inference(
                args.server_address, args.model, args.split_point, args.batch_size
            )
